...
const Block = () => {
  ...
  const removeUrl = (key) => {
    ...
    setBlockedUrls(errors => errors.filter((item, index) => item !== localUrls[key]))
  }

  const handleItemSelect = (itemId) => {
    const itemExists = blockedUrls.some(item => item === localUrls[itemId])
    if (itemExists) {
      const updatedItems = blockedUrls.filter(item => item !== localUrls[itemId]);
        ...
        {localUrls?.map((url, index) =>
          <div className={blockedUrls.some(item => item === localUrls[index]) ? 'selected block__website' : 'block__website'} key={index}>
            <button
              ...
              <span className='link-name' title={url}>{url.substring(0, url.lastIndexOf("."))}</span>
            </button>
